CREATE SCHEMA IF NOT EXISTS trip_mgmt;

DO $$ BEGIN
    CREATE TYPE trip_mgmt.trip_status AS ENUM ('DRAFT', 'DISPATCHED', 'COMPLETED', 'CANCELLED');
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;

CREATE TABLE IF NOT EXISTS trip_mgmt.trips (
    ...
    planned_distance NUMERIC(15,2) NOT NULL,
    initial_odometer NUMERIC(15,2),
    final_odometer NUMERIC(15,2),
    actual_distance NUMERIC(15,2),
    fuel_consumed NUMERIC(15,2),
    revenue NUMERIC(15,2) NOT NULL DEFAULT 0,
    status trip_mgmt.trip_status NOT NULL DEFAULT 'DRAFT',
    ...
    CONSTRAINT fk_trips_vehicle FOREIGN KEY (vehicle_id) REFERENCES vehicles.vehicles(id) ON DELETE RESTRICT,
    CONSTRAINT fk_trips_driver FOREIGN KEY (driver_id) REFERENCES drivers.drivers(id) ON DELETE RESTRICT,
    CONSTRAINT fk_trips_user FOREIGN KEY (created_by) REFERENCES users.users(id) ON DELETE RESTRICT,

    CONSTRAINT chk_trips_cargo_weight CHECK (cargo_weight > 0),
    CONSTRAINT chk_trips_planned_distance CHECK (planned_distance > 0),
    CONSTRAINT chk_trips_initial_odometer CHECK (initial_odometer IS NULL OR initial_odometer >= 0),
    CONSTRAINT chk_trips_final_odometer CHECK (final_odometer IS NULL OR final_odometer >= 0),
    CONSTRAINT chk_trips_actual_distance CHECK (actual_distance IS NULL OR actual_distance >= 0),
    CONSTRAINT chk_trips_fuel_consumed CHECK (fuel_consumed IS NULL OR fuel_consumed >= 0),
    CONSTRAINT chk_trips_revenue CHECK (revenue >= 0)
);

CREATE TABLE IF NOT EXISTS trip_mgmt.trip_status_history (
    id SERIAL PRIMARY KEY,
    trip_id INTEGER NOT NULL,
    previous_status trip_mgmt.trip_status,
    new_status trip_mgmt.trip_status NOT NULL,
    reason TEXT,
    changed_by INTEGER NOT NULL,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,

    CONSTRAINT fk_status_trip FOREIGN KEY (trip_id) REFERENCES trip_mgmt.trips(id) ON DELETE RESTRICT,
    CONSTRAINT fk_status_user FOREIGN KEY (changed_by) REFERENCES users.users(id) ON DELETE RESTRICT
);
